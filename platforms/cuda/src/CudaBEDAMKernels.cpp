...
#include "CudaBEDAMKernelSources.h"
...
CudaIntegrateLangevinStepBEDAMKernel::~CudaIntegrateLangevinStepBEDAMKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaIntegrateLangevinStepBEDAMKernel::initialize(const System& system, const LangevinIntegratorBEDAM& integrator) {
    cu.getPlatformData().initializeContexts(system);
    cu.setAsCurrent();
    cu.getIntegrationUtilities().initRandomNumberGenerator(integrator.getRandomNumberSeed());
    map<string, string> defines;
    CUmodule module = cu.createModule(CudaBEDAMKernelSources::langevin, defines, "");
    //edit on 7/7/15    
...
void CudaIntegrateLangevinStepBEDAMKernel::execute(ContextImpl& context, const LangevinIntegratorBEDAM& integrator) {
    cu.setAsCurrent();
